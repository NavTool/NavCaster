...
Item {
            ...
            Control{
                ...
                width: {
                    ...
                    if(tabWidthBehavior === TabViewType.SizeToContent){
                        var contentWidth = text_width+51
                        return contentWidth
                    }
                ...
                RowLayout{
                    ...
                    Label{
                        ...
                        {
                            item_container.text_width=width
                        }
                    }
                }
                IconButton{
                    ...
                    onClicked: {
                        ...
                        console.log("tab_model.remove(index)"+index)
                    }
                }
                Rectangle{
                    width: 1
    ...
    Item{
        ...
        Instantiator {
            ...
            delegate:HotLoader {
                ...
                PageContext{
                    id: context
                    path: pageLoader.path
                    url: pageLoader.url
                    argument: pageLoader.argument
                    singleton: pageLoader.singleton
                    view: pageLoader.stackView
                    router: pageLoader.pageRouter
                }
            }

            onObjectAdded: (index, object) => {
                               object.parent = container  // 将页面加入 container
                               object.z = index           // 控制页面层级（可选）

                               // console.log("object add!")
                           }

            onObjectRemoved: (index, object) => {
                                 // object.destroy()          // 显式销毁，释放内存
                                 // console.log("object source clear!")
                             }
        }
    ...
    function createTab(key,url,uid,icon,text,page,argument={}){
        return {key:key,url:url,uid:uid,icon:icon,text:text,page:page,argument:argument}
    }
    function appendTab(key,url,uid,icon,text,page,argument){
        tab_model.append(createTab(key,url,uid,icon,text,page,argument))
    }
