/*
    client_ntrip_co.h — 协程版本的移动站 Carrier（混合设计）
...
class client_ntrip_co : public carrier_coroutine
{
    decode_nmea _str_decoder;

public:
    client_ntrip_co(ConnectInfo info) : carrier_coroutine(info)
    {
        __class__ = "client_ntrip_co";
    }

    ~client_ntrip_co() = default;
    ...
    DetachedTask run() override
    {
        ...
        // 3. 订阅
        auto sub = co_await co_subscribe();
        ...
        while (auto evt = co_await _events.next())
        {
            ...
            case CarrierEventType::SubscribeReply:
                ...
                {
                    send_data(evt->caster_data.data(), evt->caster_data.size(), _transfer_with_chunked);
                }
