...
int client_near::runing()
{
    ...
    // // 添加一个请求，订阅指定频道数据
    // CASTER::Sub_Base_Raw_Data(_login_mpt.c_str(), _user_name.c_str(), _connect_key.c_str(), Caster_Sub_Callback, this);
...
void client_near::Auth_Login_Callback(const char *request, void *arg, auth_reply *reply)
{
    ...
    {
    case AuthReply::OK:
        CASTER::Register_Rover_Record(svr->_login_mpt.c_str(), svr->_user_name.c_str(), svr->_connect_key.c_str(), Caster_Register_Callback, svr, CasterRegisterType::NEAREST_MPT);
        break;
    case AuthReply::ERR:
...
void client_near::Caster_Sub_Callback(const char *request, void *arg, catser_reply *reply)
{
    ...
    {
        svr->_alias_mpt = reply->str;
        spdlog::info("[{}:{}]: user [{}] is start recv [{}]'s data, distance {} km, using mount [{}], addr:[{}:{}]", __class__, __func__, svr->_user_name, svr->_alias_mpt, reply->dval, svr->_login_mpt, svr->_ip, svr->_port);
    }
    else if (reply->type == CasterReply::ERR)
    {
        ...
        if (svr->_alias_mpt != "")
        {
            CASTER::Sub_Near_Raw_Data(svr->_login_mpt.c_str(), svr->_lat, svr->_lon, svr->_user_name.c_str(), svr->_connect_key.c_str(), Caster_Sub_Callback, svr);
            svr->_alias_mpt = "";
        }
        else
        {
