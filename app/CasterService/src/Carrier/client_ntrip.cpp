...
int client_ntrip::stop()
{
    ...
    CASTER::Unsub_Base_Raw_Data(_login_mpt.c_str(), _connect_key.c_str());
...
void client_ntrip::Auth_Login_Callback(const char *request, void *arg, auth_reply *reply)
{
    auto svr = static_cast<client_ntrip *>(arg);
    ...
    {
    case AuthReply::OK:
        CASTER::Register_Rover_Record(svr->_login_mpt.c_str(), svr->_user_name.c_str(), svr->_connect_key.c_str(), Caster_Register_Callback, svr, CasterRegisterType::NORMAL);
        break;
    case AuthReply::ERR:
