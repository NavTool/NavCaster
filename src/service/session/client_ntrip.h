/*
    client_ntrip.h - non-coroutine rover session.
*/
#pragma once
...
private:
    // 固定信息
    const CasterRegisterType _register_type = CasterRegisterType::CLIENT;
    const AuthType _auth_type = AuthType::CLIENT;
    std::string __class__ = "client_ntrip";
...
public:
    explicit client_ntrip(ConnectInfo info);
    ~client_ntrip();
