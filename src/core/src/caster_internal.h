...
// 最近基站订阅的位置缓存：记录上次触发 GEORADIUS 检索时的 ECEF 坐标，
// 用于距离阈值判断，避免在静止/微小漂移下频繁切换基站
struct near_pos_cache
{
    double ecef_x = 0.0;
    double ecef_y = 0.0;
    double ecef_z = 0.0;
...
private:
    ...
    std::unordered_map<std::string, near_pos_cache> _near_pos_cache_map;                                  // 连接key -> 上次触发最近基站检索时的ECEF坐标
