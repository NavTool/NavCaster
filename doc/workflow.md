...
```
GET /api/events/stream?token=<jwt>&channels=servers,clients,nodes
...
```
Login.tsx → setBaseURL(http://host:port) + login() → JWT 存 localStorage
   └─ MainLayout → 嵌套路由
