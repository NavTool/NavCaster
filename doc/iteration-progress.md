...
- 验证结果：
  ...
  - NC-031 Docker bridge 隔离网络 Cluster e2e：`deploy/scripts/e2e_smoke.ps1` 新增默认关闭的 `-IncludeDockerBridgeCluster`，独立创建 Docker bridge network、Redis 8.6.3 容器和两个 `navcaster:latest` Linux runtime 容器节点。
  ...
  - NC-031 已合并 team-dev @ 90c9b71；任务分支验证通过 AST parse、`git diff --check`、contract check、CMake+Ninja configure/build、`schema_smoke`、CTest、Web build、默认 Docker e2e 和 `-IncludeDockerBridgeCluster`；主干复验通过 AST parse、contract check、`schema_smoke.exe`、默认 Docker e2e 和 `-IncludeDockerBridgeCluster`。该任务使用本地已有 `navcaster:latest` 镜像作为隔离网络验证，不等同真实跨物理主机/网络分区；反向代理/sticky session 和 relay failover 仍由 NC-032、NC-033 覆盖。
  - NC-032 HTTP ingress / sticky session / 写路由策略 e2e：`deploy/scripts/e2e_smoke.ps1` 新增默认关闭的 `-IncludeHttpIngressStrategy`，独立创建 Docker bridge network、Redis 8.6.3 容器、两个 `navcaster:latest` Linux runtime 容器节点和 `nginx:latest` 反向代理容器。
