...
int main()
{
...

    {
        ...
        expect_eq_int(response_body["alert_events"].value("total_count", -1), 0, "operations monitor alert events empty before sync");
        response = operations.sync_operations_alert_events("202606");
        expect_eq_int(response.status_code, 200, "operations alert events sync");
        response_body = nlohmann::json::parse(response.body);
        expect_eq_int(response_body.value("synced_count", 0), 2, "operations alert events sync count");
        const std::string low_alert_id = "opsalert:202606:low_balance";
        const std::string negative_alert_id = "opsalert:202606:negative_balance";
        expect_true(response_body["events"].contains(low_alert_id), "operations alert events sync low balance");
        auto low_alert_event = operations_redis.hget(navcaster::redis_keys::operations_alert_event("202606").c_str(), low_alert_id.c_str());
        expect_eq(low_alert_event.value("status", std::string{}), "open", "operations alert event open status");
        expect_eq_int(low_alert_event.value("first_seen_time", 0), 6000, "operations alert event first seen");
        expect_eq_int(low_alert_event.value("occurrence_count", 0), 1, "operations alert event occurrence count");
        navcaster::http_api::OperationsController operations_later(operations_redis, 6200);
        response = operations_later.sync_operations_alert_events("202606");
        expect_eq_int(response.status_code, 200, "operations alert events second sync");
        low_alert_event = operations_redis.hget(navcaster::redis_keys::operations_alert_event("202606").c_str(), low_alert_id.c_str());
        expect_eq_int(low_alert_event.value("first_seen_time", 0), 6000, "operations alert event keeps first seen");
        expect_eq_int(low_alert_event.value("last_seen_time", 0), 6200, "operations alert event updates last seen");
        expect_eq_int(low_alert_event.value("occurrence_count", 0), 2, "operations alert event idempotent occurrence");
        response = operations_later.list_operations_alert_events("202606", "open");
        response_body = nlohmann::json::parse(response.body);
        expect_true(response_body.contains(low_alert_id), "operations alert events list open");
        response = operations_later.update_operations_alert_event(low_alert_id, "202606", "acknowledge", R"({"operator":"qa","operator_note":"seen"})");
        expect_eq_int(response.status_code, 200, "operations alert event acknowledge");
        response_body = nlohmann::json::parse(response.body);
        expect_eq(response_body.value("status", std::string{}), "acknowledged", "operations alert event acknowledged status");
        expect_eq(response_body.value("acknowledged_by", std::string{}), "qa", "operations alert event acknowledged by");
        response = operations_later.update_operations_alert_event(low_alert_id, "202606", "resolve", R"({"operator":"qa","operator_note":"closed"})");
        expect_eq_int(response.status_code, 200, "operations alert event resolve");
        response_body = nlohmann::json::parse(response.body);
        expect_eq(response_body.value("status", std::string{}), "resolved", "operations alert event resolved status");
        response = operations_later.update_operations_alert_event(low_alert_id, "202606", "acknowledge", R"({"operator":"qa"})");
        expect_eq_int(response.status_code, 409, "operations resolved alert cannot acknowledge");
        navcaster::http_api::OperationsController operations_reopen(operations_redis, 6300);
        response = operations_reopen.sync_operations_alert_events("202606");
        expect_eq_int(response.status_code, 200, "operations alert events reopen sync");
        low_alert_event = operations_redis.hget(navcaster::redis_keys::operations_alert_event("202606").c_str(), low_alert_id.c_str());
        expect_eq(low_alert_event.value("status", std::string{}), "open", "operations alert event reopens");
        expect_eq_int(low_alert_event.value("reopen_count", 0), 1, "operations alert event reopen count");
        response = operations_reopen.update_operations_alert_event(negative_alert_id, "", "resolve", R"({"operator":"qa"})");
        expect_eq_int(response.status_code, 200, "operations alert event resolves with period from id");
        response = operations_reopen.update_operations_alert_event("opsalert:202606:missing", "202606", "resolve", R"({})");
        expect_eq_int(response.status_code, 404, "operations alert event missing");
        response = operations_reopen.update_operations_alert_event(low_alert_id, "202606", "snooze", R"({})");
        expect_eq_int(response.status_code, 400, "operations alert event invalid action");
        response = operations_reopen.list_operations_alert_events("202606", "resolved");
        response_body = nlohmann::json::parse(response.body);
        expect_true(response_body.contains(negative_alert_id), "operations alert events list resolved");
        response = operations_reopen.operations_monitor("202606");
        response_body = nlohmann::json::parse(response.body);
        expect_true(response_body["alert_events"].value("total_count", 0) >= 2, "operations monitor alert event summary count");
        expect_true(!response_body["alert_events"]["recent_events"].empty(), "operations monitor alert event summary recent");
