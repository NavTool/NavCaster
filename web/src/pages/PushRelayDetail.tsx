...
const PushRelayDetail: React.FC = () => {
  const { id } = useParams<{ id: string }>();
  const navigate = useNavigate();
  const { data: record, refresh: refreshRecord } = usePolling<PushRecord | null>(
  ...
  const handleToggle = async () => {
    if (!id) return;
    if (running) await relayStop('push', id);
    else await relayStart('push', id);
    refreshRecord();
    refreshState();
  };

  if (!id || !record) {
    return <Card style={{ borderColor: '#2e3450' }}><Empty description="未找到 Push 中继" /></Card>;
  }

  return (
    <div>
      <Breadcrumb items={[{ title: <Link to="/relay/push">Push 中继</Link> }, { title: record.login_mpt || id }]} style={{ marginBottom: 16 }} />
      ...
      <Card title="操作" style={{ borderColor: '#2e3450' }}>
        <Space>
          ...
          <Button onClick={() => navigate('/relay/push')}>返回列表</Button>
...
export default PushRelayDetail;